   ...
   if(loadedData.length == 0)
   {
    ...
    if(errorData)
    {
        return  (
            ...
            <div   className={"container " + mode.bodyMode}>
            <Filter/>
             <h1 className={"no-country-found  " + mode.bodyMode} style={{display:"flex",flexDirection:"column" ,alignItems:'center' , rowGap:'50px'}}>something went wrong .Please try again <br /> <button id="back"  className={mode.elementMode} style={{margin:'0px auto'}}>Try Again</button></h1>
           </div>
           </ApplyFilter.Provider>
        )
    }
